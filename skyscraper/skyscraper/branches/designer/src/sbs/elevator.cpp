...
void Elevator::MoveElevatorToFloor()
{
	...
	if (Brakes == false)
	{
		...
		if (Direction == 1 && ElevatorRate < limit)
			ElevatorRate += UpSpeed * ((Acceleration * JerkRate) * sbs->delta);
		else if (Direction == -1 && ElevatorRate > -limit)
			ElevatorRate -= DownSpeed * ((Acceleration * JerkRate) * sbs->delta);
		else
			CalculateStoppingDistance = false;
	}
	else if (Leveling == false)
	{
		...
		if (Direction == 1)
			ElevatorRate += UpSpeed * ((TempDeceleration * JerkRate) * sbs->delta);
		if (Direction == -1)
			ElevatorRate -= DownSpeed * ((TempDeceleration * JerkRate) * sbs->delta);
	}
